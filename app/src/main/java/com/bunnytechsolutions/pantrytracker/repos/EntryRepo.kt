...
class EntryRepo(private val dbHelper: DBHelper) {
    ...
    fun insertEntry(pantryUUID: String, entry_: Entry): Long {
        val entry = entry_.toBuilder().setId(UUID.randomUUID().toString()).build()
        ...
        contentValues.put(DBHelper.COL_ENTRY_OBJECT, entry.toByteArray())
    ...
    fun getEntryByUUID(entryUUID: String): Entry? {
    ...
    fun getEntriesByPantryUUID(pantryUUID: String) : Array<Entry?> {
        val db = dbHelper.readableDatabase
    ...
    fun getEntriesByPantryUUIDAndProductCode(pantryUUID: String, productCode: String) : Array<Entry?> {
        val db = dbHelper.readableDatabase
    ...
    fun getExpiredEntriesByPantryUUID(pantryUUID: String, timeMillis: Long) : Array<Entry?> {
        val db = dbHelper.readableDatabase
    ...
    @SuppressLint("Range")
    private fun parseEntriesFromCursor(cursor: Cursor) : Array<Entry?> {
        val result = arrayOfNulls<Entry>(cursor.count)
